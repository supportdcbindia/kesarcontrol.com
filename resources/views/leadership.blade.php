@extends('layouts.header')
@section('content')
  <section>
    <div class="breadcrumb-cst director-banner px" data-aos>
      <div class="container-fluid">
        <div class="row row-cols-1 row-cols-lg-2 align-items-center">
          <div class="col">
            <div class="left">
              <div class="heading">
                <h1 data-aos="fade-up">Our Visionary Leadership
                </h1>
              </div>
              <div class="para">
                <p data-aos="fade-up" data-aos-delay="200">Driven by expertise and innovation, our leadership team
                  shapes Kesar Control's
                  journey to excellence.</p>
              </div>
            </div>
          </div>
          <div class="col">
            <div class="right" data-aos="fade-up" data-aos-delay="300">
              <nav>
                <ol class="breadcrumb">
                  <li class="breadcrumb-item">
                    <a href="/">
                      <svg width="31" height="24" viewBox="0 0 31 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path
                          d="M15.089 6.22623L5.16708 14.362V23.1428C5.16708 23.3701 5.25779 23.5882 5.41927 23.7489C5.58075 23.9097 5.79976 24 6.02812 24L12.0587 23.9845C12.2863 23.9833 12.5042 23.8925 12.6647 23.7319C12.8253 23.5712 12.9154 23.3538 12.9154 23.1272V17.9993C12.9154 17.772 13.0061 17.5539 13.1676 17.3932C13.3291 17.2324 13.5481 17.1421 13.7764 17.1421H17.2206C17.449 17.1421 17.668 17.2324 17.8295 17.3932C17.991 17.5539 18.0817 17.772 18.0817 17.9993V23.1235C18.0813 23.2363 18.1033 23.348 18.1464 23.4523C18.1896 23.5567 18.2529 23.6515 18.3329 23.7314C18.4129 23.8113 18.5079 23.8746 18.6126 23.9179C18.7172 23.9611 18.8294 23.9834 18.9427 23.9834L24.9711 24C25.1995 24 25.4185 23.9097 25.58 23.7489C25.7414 23.5882 25.8321 23.3701 25.8321 23.1428V14.3561L15.9124 6.22623C15.7958 6.13264 15.6505 6.08161 15.5007 6.08161C15.3509 6.08161 15.2056 6.13264 15.089 6.22623ZM30.7616 11.756L26.2627 8.06394V0.642929C26.2627 0.472414 26.1946 0.308882 26.0735 0.18831C25.9524 0.0677371 25.7882 0 25.6169 0H22.6032C22.432 0 22.2677 0.0677371 22.1466 0.18831C22.0255 0.308882 21.9574 0.472414 21.9574 0.642929V4.53318L17.1394 0.586673C16.677 0.207869 16.0968 0.000757234 15.498 0.000757234C14.8992 0.000757234 14.319 0.207869 13.8566 0.586673L0.234365 11.756C0.168973 11.8098 0.114871 11.8759 0.0751508 11.9505C0.0354308 12.0251 0.0108711 12.1068 0.0028749 12.1909C-0.00512131 12.275 0.00360275 12.3598 0.0285486 12.4406C0.0534944 12.5213 0.0941732 12.5963 0.14826 12.6614L1.52055 14.3223C1.57449 14.3876 1.64084 14.4417 1.71579 14.4814C1.79075 14.5212 1.87284 14.5458 1.95736 14.5539C2.04189 14.562 2.1272 14.5534 2.2084 14.5287C2.28959 14.5039 2.36509 14.4635 2.43057 14.4096L15.089 4.02956C15.2056 3.93597 15.3509 3.88494 15.5007 3.88494C15.6505 3.88494 15.7958 3.93597 15.9124 4.02956L28.5714 14.4096C28.6367 14.4635 28.7121 14.504 28.7932 14.5288C28.8743 14.5537 28.9595 14.5623 29.0439 14.5544C29.1284 14.5464 29.2104 14.522 29.2854 14.4824C29.3604 14.4429 29.4268 14.389 29.4808 14.3239L30.8531 12.663C30.9071 12.5976 30.9477 12.5221 30.9723 12.4411C30.997 12.36 31.0053 12.2748 30.9968 12.1906C30.9883 12.1063 30.9631 12.0245 30.9228 11.9499C30.8824 11.8753 30.8277 11.8094 30.7616 11.756Z"
                          fill="white" />
                      </svg>
                      Home
                    </a>
                  </li>
                  <li class="breadcrumb-item active" aria-current="page"><a href="about-us"><span>About
                        Us</span></a></li>
                  <li class="breadcrumb-item active" aria-current="page"><span>Director Desk</span></li>
                </ol>
              </nav>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
  <section>
    <div class="px py director-desk">
      <div class="container-fluid">
        <div class="row g">
          <div class="col-10 col-md-6 col-lg-auto mx-auto">
            <div class="img type-one" data-aos="zoom-in">
              <img loading="lazy" src="asset/img/experts/1-min.jpg" alt="" class="img-fluid lazy">
            </div>
          </div>
          <div class="col-lg">
            <div class="row row-cols-1">
              <div class="col mx-auto">
                <div class="main-heading bold">
                  <h2 class="" data-aos="fade">
                    Mehul Patel
                  </h2>
                </div>
              </div>
              <div class="col">
                <div class="title" data-aos="fade-up" data-aos-delay="100">
                  <h3 class="heading mb">
                    <span>
                      Founder & Director
                    </span>
                  </h3>
                </div>
              </div>
            </div>
            <div class="row">
              <div class="col">
                <div class="para lg mt" data-aos="fade-up" data-aos-delay="600">
                  <p>
                    “At Kesar Control Systems, our unwavering commitment is to deliver top-tier pharmaceutical
                    and laboratory equipment that aligns with the highest standards of quality, innovation, and
                    reliability. Since our inception in 2007, we have continuously worked to exceed customer
                    expectations through our dedication to excellence, technological advancement, and continuous
                    improvement.
                  </p>
                  <p>
                    With over a decade of experience and a strong focus on precision-engineered solutions, we aim
                    to meet the dynamic and evolving demands of the pharmaceutical and scientific industries. Our
                    products are developed with meticulous attention to detail and a deep understanding of
                    compliance, safety, and performance.
                  </p>
                  <p>
                    We believe in building lasting partnerships with our clients, founded on trust, transparency, and
                    unmatched service, to
                    ensure satisfaction at every stage of their journey with us.</p>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
  <section>
    <div class="px pb director-desk-all">
      <div class="container-fluid">
        <div class="row row-cols-1">
          <div class="col">
            <div class="title center" data-aos="fade">
              <div class="before lg"></div>
              <h3 class="heading mb">
                <span>
                  Kesar Team
                </span>
              </h3>
              <div class="after lg"></div>
            </div>
          </div>
          <div class="col mx-auto">
            <div class="main-heading bold center">
              <h2 class="" data-aos="fade-up" data-aos-delay="100">
                Leadership That Inspires
              </h2>
            </div>
          </div>
        </div>
        <div class="row row-cols-1 row-cols-md-2 g mt">
          <div class="col-lg-4">
            <div class="card ava-card">
              <div class="row">
                <div class="col-10 col-md-5 mx-auto">
                  <div class="img type-one" data-aos="zoom-in" data-aos-delay="400">
                    <img loading="lazy" class="lazy img-fluid" src="asset/img/experts/2-min.jpg" alt="">
                  </div>
                </div>
              </div>

              <div class="card-body">
                <div class="details">
                  <div class="name" data-aos="fade">
                    Mr. Jagdish Patel
                  </div>
                  <div class="position" data-aos="fade-up" data-aos-delay="100">
                    Managing Director
                  </div>
                </div>
              </div>
              <div class="card-footer">
                <div class="row">
                  <div class="col-lg-10 mx-auto">
                    <div class="para" data-aos="fade-up" data-aos-delay="600">
                      <p>
                        "At Kesar, we strive to blend innovation with reliability,
                        ensuring every solution we deliver adds lasting value to
                        our clients' operations."
                      </p>
                    </div>
                  </div>
                </div>

              </div>
            </div>
          </div>
          <div class="col-lg-4">
            <div class="card ava-card">
              <div class="row">
                <div class="col-10 col-md-5 mx-auto">
                  <div class="img type-one" data-aos="zoom-in" data-aos-delay="400">
                    <img loading="lazy" class="lazy img-fluid" src="asset/img/experts/3-min.jpg" alt="">
                  </div>
                </div>
              </div>

              <div class="card-body">
                <div class="details">
                  <div class="name" data-aos="fade">
                    Mr. Kalpen Patel
                  </div>
                  <div class="position" data-aos="fade-up" data-aos-delay="100">
                    CEO
                  </div>
                </div>
              </div>
              <div class="card-footer">
                <div class="row">
                  <div class="col-lg-10 mx-auto">
                    <div class="para" data-aos="fade-up" data-aos-delay="600">
                      <p>
                        "Our vision is to set new benchmarks in quality and service
                        while fostering a culture of growth, trust, and continuous
                        improvement."
                      </p>
                    </div>
                  </div>
                </div>

              </div>
            </div>
          </div>
          <div class="col-lg-4">
            <div class="card ava-card">
              <div class="row">
                <div class="col-12 col-md-10 mx-auto">
                  <div class="img type-one" data-aos="zoom-in" data-aos-delay="400">
                    <img loading="lazy" class="lazy img-fluid" src="asset/img/experts/4-min.jpg" alt="">
                  </div>
                </div>
              </div>

              <div class="card-body">
                <div class="details">
                  <div class="name" data-aos="fade">
                    Kesar Team
                  </div>
                  <div class="position" data-aos="fade-up" data-aos-delay="100">
                    Innovators at Work

                  </div>
                </div>
              </div>
              <div class="card-footer">
                <div class="row">
                  <div class="col-lg-10 mx-auto">
                    <div class="para" data-aos="fade-up" data-aos-delay="600">
                      <p>
                        Success is best achieved when the whole team is strong.
                      </p>
                    </div>
                  </div>
                </div>

              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
@endsection